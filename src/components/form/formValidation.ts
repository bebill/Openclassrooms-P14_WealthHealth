...
export const validateForm = (values: Employee) => {
  ...
  if (firstName === "") {
    ...
    errors.firstName = "First name should be min 2 characters";
  } else if (!regex.test(firstName)) {
    errors.firstName = "First name can only contain letters, spaces and '-";
  }

  if (lastName === "") {
    errors.lastName = "Please enter a last name";
  } else if (lastName.length < 2) {
    errors.lastName = "Last name should be min 2 characters";
  ...
  if (street === "") {
    ...
    errors.street = "Street name should be min 2 characters";
  ...
  if (city === "") {
    ...
    errors.city = "City name should be min 2 characters";
  } else if (!regex.test(city)) {
    errors.city = "City name can only contain letters, spaces and '-";
  }

  if (zip === "") {
    errors.zip = "Please enter a Zipcode";
  } else if (!/^\d+$/.test(zip)) {
